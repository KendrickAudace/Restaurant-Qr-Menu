...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0"><i class="fas fa-fw fa-xs fa-store text-primary-900 mr-2"></i> <?= l('admin_stores.header') ?></h1>
                    ...
                    <form action="" method="get" role="form">
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="order_by" id="filters_order_by" class="custom-select custom-select-sm">
                                <option value="store_id" <?= $data->filters->order_by == 'store_id' ? 'selected="selected"' : null ?>><?= l('global.id') ?></option>
...
<form id="table" action="<?= SITE_URL . 'admin/stores/bulk' ?>" method="post" role="form">
    ...
    <div class="table-responsive table-custom-container">
        <table class="table table-custom">
            ...
            <tr>
                ...
                <th><?= l('global.user') ?></th>
                <th><?= l('admin_stores.table.name') ?></th>
                <th><?= l('admin_stores.table.stats') ?></th>
            ...
            <?php foreach($data->stores as $row): ?>
                ...
                <tr>
                    <td data-bulk-table class="d-none">
                        <div class="custom-control custom-checkbox">
                            <input id="selected_store_id_<?= $row->store_id ?>" type="checkbox" class="custom-control-input" name="selected[]" value="<?= $row->store_id ?>" />
                            <label class="custom-control-label" for="selected_store_id_<?= $row->store_id ?>"></label>
                        </div>
                    </td>
                    ...
                    <td class="text-nowrap">
                        <?= $row->name ?><a href="<?= url('store-redirect?store_id=' . $row->store_id) ?>" rel="noreferrer"><i class="fas fa-fw fa-xs fa-external-link-alt ml-1"></i></a>
                    </td>
                    <td class="text-nowrap">
                        <div class="d-flex flex-column">
                            <span class="text-muted"><?= sprintf(l('admin_stores.table.pageviews'), nr($row->pageviews)) ?></span>
                            <span class="text-muted"><?= sprintf(l('admin_stores.table.orders'), nr($row->orders)) ?></span>
                        </div>
                    </td>
                    <td class="text-nowrap">
                    ...
                    <td>
                        <div class="d-flex justify-content-end">
                            <?= include_view(THEME_PATH . 'views/admin/stores/admin_store_dropdown_button.php', ['id' => $row->store_id, 'resource_name' => $row->name]) ?>
                        </div>
                    </td>
                </tr>
            <?php endforeach ?>

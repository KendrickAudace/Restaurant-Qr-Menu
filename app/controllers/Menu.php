...
class Menu extends Controller {
    ...
    public function duplicate() {
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            $menu_id = db()->insert('menus', [
                ...
                'url' => $url,
                'name' => string_truncate($menu->name . ' - ' . l('global.duplicated'), 64, null),
            ...
            foreach($categories as $category) {
                ...
                foreach($items as $item) {
                    ...
                    (new \Altum\Models\Item())->duplicate($item, $item_id, $category_id, $menu_id, $item->store_id, $this->user->user_id);
                }
            }
    ...
    public function delete() {
        ...
        /* Team checks */
        if(\Altum\Teams::is_delegated() && !\Altum\Teams::has_access('delete.menus')) {
            Alerts::add_info(l('global.info_message.team_no_access'));
            redirect('dashboard');
        }
        ...
        if(!$menu = db()->where('user_id', $this->user->user_id)->where('menu_id', $menu_id)->getOne('menus', ['store_id', 'menu_id', 'image', 'name'])) {
            redirect('dashboard');
        }
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            /* Clear cache */
            cache()->deleteItemsByTag('store_id=' . $menu->store_id);

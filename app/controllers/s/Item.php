...
class Item extends Controller {
    ...
    public function index() {
        ...
        /* Set a custom title */
        Title::set(sprintf(l('s_item.title'), $this->item->name, $this->category->name, $this->menu->name, $this->store->name));

        /* Set a custom title */
        if($this->store->settings->title) {
            Title::set(sprintf(l('s_item.title'), $this->item->name, $this->category->name, $this->menu->name, $this->store->settings->title), true);
        ...
        /* Set the meta tags */
        Meta::set_description(string_truncate($this->item->description, 160));
        Meta::set_social_url($this->store->full_url . $this->menu->url . '/' . $this->category->url . '/' . $this->item->url);
        Meta::set_social_title(sprintf(l('s_item.title'), $this->item->name, $this->category->name, $this->menu->name, $this->store->name));
        Meta::set_social_description(string_truncate($this->item->description, 160));
        Meta::set_social_image($this->item->image ? \Altum\Uploads::get_full_url('item_images') . $this->item->image : null);
        Meta::set_canonical_url($this->store->full_url . $this->menu->url . '/' . $this->category->url . '/' . $this->item->url);
        ...
        $view = new \Altum\View('s/item/' . $this->store->theme . '/index', (array) $this);
    ...
    public function init($store_id = null) {
        ...
        if(isset(\Altum\Router::$data['domain']) && \Altum\Router::$data['domain']->store_id) {
            ...
            $url = isset($this->params[3]) ? query_clean($this->params[3]) : null;
        }

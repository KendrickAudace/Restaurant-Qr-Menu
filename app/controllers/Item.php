...
class Item extends Controller {
    ...
    public function duplicate() {
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            $item_id = db()->insert('items', [
                ...
                'url' => $url,
                'name' => string_truncate($item->name . ' - ' . l('global.duplicated'), 64, null),
            ...
            (new \Altum\Models\Item())->duplicate($item, $item_id, $item->category_id, $item->menu_id, $item->store_id, $this->user->user_id);
    ...
    public function delete() {
        ...
        /* Team checks */
        if(\Altum\Teams::is_delegated() && !\Altum\Teams::has_access('delete.items')) {
            Alerts::add_info(l('global.info_message.team_no_access'));
            redirect('dashboard');
        }
        ...
        if(!$item = db()->where('user_id', $this->user->user_id)->where('item_id', $item_id)->getOne('items', ['store_id', 'menu_id', 'category_id', 'item_id', 'image', 'name'])) {
            redirect('dashboard');
        }
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            /* Clear cache */
            cache()->deleteItemsByTag('store_id=' . $item->store_id);

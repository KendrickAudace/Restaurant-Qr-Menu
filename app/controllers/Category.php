...
class Category extends Controller {
    ...
    public function duplicate() {
        ...
        /* Team checks */
        if(\Altum\Teams::is_delegated() && !\Altum\Teams::has_access('create.categories')) {
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            $category_id = db()->insert('categories', [
                ...
                'url' => $url,
                'name' => string_truncate($category->name . ' - ' . l('global.duplicated'), 64, null),
            ...
            foreach($items as $item) {
                ...
                (new \Altum\Models\Item())->duplicate($item, $item_id, $category_id, $item->menu_id, $item->store_id, $this->user->user_id);
            }
    ...
    public function delete() {
        ...
        /* Team checks */
        if(\Altum\Teams::is_delegated() && !\Altum\Teams::has_access('delete.categories')) {
            Alerts::add_info(l('global.info_message.team_no_access'));
            redirect('dashboard');
        }
        ...
        if(!$category = db()->where('user_id', $this->user->user_id)->where('category_id', $category_id)->getOne('categories', ['store_id', 'menu_id', 'category_id', 'name'])) {
            redirect('dashboard');
        }
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            /* Clear cache */
            cache()->deleteItemsByTag('store_id=' . $category->store_id);
    ...
    public function order_ajax() {
        ...
        /* Team checks */
        if(\Altum\Teams::is_delegated() && !\Altum\Teams::has_access('update.categories')) {
            die();
        }

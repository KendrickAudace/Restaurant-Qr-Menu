...
class ItemVariantUpdate extends Controller {
    ...
    public function index() {
        ...
        if(!$item_variant = db()->where('item_variant_id', $item_variant_id)->where('user_id', $this->user->user_id)->getOne('items_variants', '*')) {
            redirect('dashboard');
        }
        ...
        $item = db()->where('item_id', $item_variant->item_id)->where('user_id', $this->user->user_id)->getOne('items', ['item_id', 'url']);

        $category = db()->where('category_id', $item_variant->category_id)->where('user_id', $this->user->user_id)->getOne('categories', ['category_id', 'url']);

        $menu = db()->where('menu_id', $item_variant->menu_id)->where('user_id', $this->user->user_id)->getOne('menus', ['menu_id', 'url']);

        $store = db()->where('store_id', $item_variant->store_id)->where('user_id', $this->user->user_id)->getOne('stores', ['store_id', 'domain_id', 'url', 'currency']);
